 ...
 *---------------------------------------------------------------------
 * Adaptation Webi-Time : NoLife4Ever
 * Interface partagée : WebiTime_GT_Common.js
 *---------------------------------------------------------------------*/
...
(async function() {
    ...
    async function ensureWebiTimeCommon() {
        if (window.WebiTimeGT && typeof window.WebiTimeGT.injectResourceStyles === 'function') {
        ...
        for (const url of candidates) {
            ...
            try {
                ...
                console.warn('[Webi-Time Ressources Entrantes] Echec du chargement commun :', url, error);
            }
        }
        return null;
    }
    ...
    if (!WEBITIME_UI) {
        console.error('[Webi-Time Ressources Entrantes] Impossible de charger WebiTime_GT_Common.js.');
    ...
    const WEBITIME_RESOURCE_GITHUB_ISSUES_URL = WEBITIME_UI.buildIssueUrl('GT-RessourceEntrante', WEBITIME_SOURCE_URL);
    ...
    if (!TwCheese.hasTool(toolId)) {
        ...
        function clearSavedResourceData() {
            try {
                if (userConfig && userConfig.props) {
                    const haulStats = userConfig.props.HaulStatsWidget;
                    if (haulStats && Object.prototype.hasOwnProperty.call(haulStats, 'collapseStats')) {
                        delete haulStats.collapseStats;
                    }
                    if (haulStats && Object.keys(haulStats).length === 0) delete userConfig.props.HaulStatsWidget;

                    const redirect = userConfig.props.suggestRedirect;
                    const skip = redirect && redirect.skip;
                    if (skip && Object.prototype.hasOwnProperty.call(skip, 'Tool:OverviewHauls')) {
                        delete skip['Tool:OverviewHauls'];
                    }
                ...
                console.warn('[Webi-Time Ressources Entrantes] Impossible de nettoyer les données enregistrées.', error);
            }
        }

        function bindResourceSettings() {
            WEBITIME_UI.bindSettingsPopover({
                containerSelector: '#twcheese_pillaging_stats',
                clearButtonSelector: '#wtri_clear_saved_data',
                bugButtonSelector: '#wtri_report_bug',
                bugUrl: WEBITIME_RESOURCE_GITHUB_ISSUES_URL,
                onClear: clearSavedResourceData,
        ...
        class HaulStatsWidget_HaulStatsWidget extends AbstractWidget["a" /* AbstractWidget */] {
            ...
            createHtml() {
                ...
                // Si aucune commande ne dépasse l'heure courante, on garde malgré tout une plage exploitable.
                if (!summationFromOptions.length) {
                    const currentHour = TwCheeseDate_TwCheeseDate.newServerDate().startOfHour();
                    const currentEnd = currentHour.endOfHour();
                    const hourOfDay = currentHour.getServerHours();
                    const dayHint = this.dayHint(currentHour);
                    summationFromOptions.push(`<option value="${currentHour.getTime()}">${hourOfDay}:00${dayHint}</option>`);
                    summationToOptions.push(`<option value="${currentEnd.getTime()}">${hourOfDay}:59${dayHint}</option>`);
                }
                                    ...
                                    <div class="wtri-table-wrap">
                                        <table class="wtri-table twcheese-pillaging-stats-hourly-breakdown">
                                            <thead>
                                                <tr>
                                                    <th>Arrivée</th>
                                                    <th><img src="${ImageSrc["a" /* ImageSrc */].wood}" alt="Bois"></th>
                                                    <th><img src="${ImageSrc["a" /* ImageSrc */].stone}" alt="Argile"></th>
                                                    <th><img src="${ImageSrc["a" /* ImageSrc */].iron}" alt="Fer"></th>
                                                    <th>Butin</th>
                                                    <th>Performance</th>
                                                </tr>
                                            </thead>
                                            <tbody>${hourlyBreakdowns.join('')}</tbody>
                                        </table>
                                    </div>
                                </div>
                            </div>
             ...
             */
            dayHint(date) {
                if (date.isTodayOnServer()) return '';
                if (date.isTomorrowOnServer()) return ' (demain)';
            ...
            updateSum() {
                ...
                this.$sum.html(`
                    <span class="wtri-result-item"><img src="${ImageSrc["a" /* ImageSrc */].wood}" alt="Bois"> ${sum.haul.wood}</span>
                    <span class="wtri-result-item"><img src="${ImageSrc["a" /* ImageSrc */].stone}" alt="Argile"> ${sum.haul.stone}</span>
                    <span class="wtri-result-item"><img src="${ImageSrc["a" /* ImageSrc */].iron}" alt="Fer"> ${sum.haul.iron}</span>
                    <span class="wtri-result-performance">Butin ${sum.sumLoot()}/${sum.haulCapacity} • ${sum.calcHaulPercent()}%</span>
                `);
            }
         ...
         */
        async function promptLoadHauls(onConfirm) {
            const dialog = WEBITIME_UI.showProgressDialog({
                rootId: 'webiTimeIncomingResourcesProgress',
                eventNamespace: 'webiTimeIncomingResourcesProgress',
                title: 'Chargement des ressources entrantes',
                message: 'Analyse des commandes de retour et récupération des pillages en cours...'
            });

            let progressMonitor = new ProgressMonitor();
            progressMonitor.onChange(e => {
                dialog.update(e.progress, e.goal, 'Commandes analysées');
            });

            try {
                await onConfirm(progressMonitor);
                dialog.update(progressMonitor.goal, progressMonitor.goal, 'Analyse terminée');
                dialog.close();
            } catch (error) {
                dialog.close();
                console.error('[Webi-Time Ressources Entrantes] Erreur pendant le chargement.', error);
                if (window.UI && window.UI.ErrorMessage) {
                    window.UI.ErrorMessage('Impossible de charger toutes les ressources entrantes.');
                }
                throw error;
            }
        }
        ...
        function suggestRedirect(options) {
            ...
            message = message || '<p>Ce script doit être lancé depuis une autre interface.</p>';
            screenName = screenName || 'l’interface demandée';
            uriParams = uriParams || {};
            if (!screen) throw Error('Un écran doit être spécifié !');

            WEBITIME_UI.showRedirectDialog({
                rootId: 'webiTimeRedirectModalRoot',
                eventNamespace: 'webiTimeIncomingResourcesRedirect',
        ...
        async function twAlert(message = '', buttonText = 'Fermer') {
            return WEBITIME_UI.showMessageDialog({
                rootId: 'webiTimeIncomingResourcesMessage',
                eventNamespace: 'webiTimeIncomingResourcesMessage',
                title: WEBITIME_RESOURCE_NAME,
                message,
                buttonText
            });
        }
        ...
        async function alertPremiumRequired() {
            return WEBITIME_UI.showMessageDialog({
                rootId: 'webiTimeIncomingResourcesPremium',
                eventNamespace: 'webiTimeIncomingResourcesPremium',
                title: 'Compte Premium requis',
                message: '<p>Ce script utilise des fonctionnalités disponibles uniquement avec un compte <b>Premium</b>.</p>',
                buttonText: 'Compris'
            });
        }
        ...
        function parseArrival(text, market) {
            ...
            switch (market) {
                ...
                case 'hu': return parseArrivalHungarian(text);	
                case 'fr': return parseArrivalFR(text);			
            }
        ...
        function parseArrivalHungarian(text) {
            // e.g. "márc 31, 2020  22:13:49:000"
            let expr = /(\D+). (\d+), (\d+)  (\d+):(\d+):(\d+):?(\d+)?/;
            var [, monthName, day, year, hours, minutes, seconds, millis] = text.match(expr);
            switch (monthName) {
                case 'jan': monthName = "jan";
                break;
                case 'febr': monthName = "feb";
                break;
                case 'márc': monthName = "mar";
                break;
                case 'ápr': monthName = "apr";
                break;
                case 'máj': monthName = "may";
                break;
                case 'jún': monthName = "jun";	
                break;
                case 'júl': monthName = "jul";
                break;
                case 'aug': monthName = "aug";
                break;
                case 'szept': monthName = "sep";
                break;
                case 'okt': monthName = "oct";
                break;
                case 'nov': monthName = "nov";
                break;
                case 'dec': monthName = "dec";
                break;
            }
            let month = TwCheeseDate_TwCheeseDate.monthNumber(monthName);
            return TwCheeseDate_TwCheeseDate.newServerDate(year, month, day, hours, minutes, seconds, millis || 0);
        }
        function parseArrivalFR(text) {
            // e.g. "márc 31, 2020  22:13:49:000"
            let expr = /(\D+) (\d+), (\d+)  (\d+):(\d+):(\d+):?(\d+)?/;
            var [, monthName, day, year, hours, minutes, seconds, millis] = text.match(expr);
            switch (monthName) {
                ...
                case 'febr': monthName = "feb";
                break;
                case 'mars': monthName = "mar";
                break;
                case 'avril': monthName = "apr";
                break;
                case 'mai': monthName = "may";
                break;
                case 'juin': monthName = "jun";	
                break;
                case 'jullet': monthName = "jul";
                break;
                case 'aug': monthName = "aug";
                break;
                case 'szept': monthName = "sep";
        ...
        function scrapeCommand(gameDoc) {
            ...
            console.log($(rowMap.arrival.cells[1]).text());
        ...
        async function enhanceScreenWithHaulInfo(progressMonitor) {
            ...
            const widget = new HaulStatsWidget_HaulStatsWidget(returningCommands, scrapePageNumber());
            widget.insertAfter($('.modemenu:eq(1)'));
            bindResourceSettings();
        ...
        function suggestRedirectToCommandsOverview() {
            suggestRedirect({
                message: `
                    <p>Pour utiliser <b>${WEBITIME_RESOURCE_NAME}</b>, tu dois être sur l’interface des ordres.</p>
                    <p>Le script doit être lancé depuis l’onglet <b>Retours</b>, car ce sont ces commandes qui ramènent les ressources.</p>`,
                screen: 'overview_villages',
                screenName: 'Ordres - Retours',
                uriParams: {
                    mode: 'commands',
                    type: 'return'
                },
        ...
        function useTool() {
            ...
            if (haulsIncluded) {
                window.UI.InfoMessage('Les ressources entrantes sont déjà affichées.', 3000);
                return;
            }
            if (!window.premium) {
            ...
            if (!document.getElementById('commands_table')) {
                window.UI.ErrorMessage('Aucune commande de retour à analyser sur cette page.', 3000);
                return;
            }
        ...
        // La barre latérale TwCheese n'est plus ouverte automatiquement :
        // les paramètres et le signalement de bug sont intégrés à l'interface Webi-Time.
    }
